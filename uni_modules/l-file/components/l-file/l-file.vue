<template>
  <view>
    ...
    <!-- #ifdef H5 -->
    <h5-file v-if="showH5" ref="h5File" :logo="logo" :showTip="showTip" :progress="value" @abort="onAbort" @close="onCloseH5"></h5-file>
    <!-- #endif -->
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    handleH5Upload({ url, name = 'file', header, addName, addSize, ...data } = {}, tempFile) {
      ...
      this.uploadTask.upload.addEventListener(
        'progress',
        (event) => {
    ...
    plusSaveFile({ url, customName = '', opt }) {
      return new Promise((resolve, reject) => {
        ...
        this.downloadTask.addEventListener(
          'statechanged',
