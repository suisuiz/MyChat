...
<template>
  <view v-if="pageLoad" class="chat-page">
              ...
              <view v-else class="other_chat">
                ...
                <view class="content-box">
                  ...
                  <block v-if="item.content.type == '1'">
                    ...
                    <!-- 投屏视频（播放停止两个按钮）subtype=7 -->
                    <view v-if="item.content.subtype == '7'" class="leftMsg_img" @click="toVideo(item.content)">
                      <image class="img" :src="item.content.cover" mode="aspectFill"></image>
                      <view class="img-box">
                        <image class="img1" v-if="item.content.bol" src="/static/img/plus/pause.png"></image>
                        <image class="img1" v-else src="/static/img/plus/play.png"></image>
                      </view>
                    </view>
                    <!-- 投屏文档（上下左右四个按钮） subtype=8 -->
                    <view v-if="item.content.subtype == '8'" class="leftMsg_img turn_img">
                      <image class="img" :src="item.content.cover" mode="aspectFill"></image>
                      <view class="img-box">
                        <view class="ups turn_center">
                          <image class="img2" @tap="turnPage('up', item.content)" src="/static/img/plus/up.png"></image>
                        </view>
                        <view class="downs turn_center">
                          <image class="img2" @tap="turnPage('down', item.content)" src="/static/img/plus/down.png"></image>
                        </view>
                        <view class="lefts turn_center">
                          <image class="img3" @touchstart="turnPage('left', item.content)" @touchend="stopPage" src="/static/img/plus/left.png"></image>
                        </view>
                        <view class="rights turn_center">
                          <image class="img3" @touchstart="turnPage('right', item.content)" @touchend="stopPage" src="/static/img/plus/right.png"></image>
                        </view>
                      </view>
                    </view>
                  </block>
    ...
    <!-- 商品收藏/转发 -->
    <view class="relay-mark uni-flex-center" v-if="isGoodsMask" @click="isGoodsMask = false">
      <view class="uni-flex-jus">
        <view class="relay-wrap uni-column-center" @click="goodsCollect">
          <image class="relay-img" src="/static/img/device/device_share1.png" mode=""></image>
          <text>收藏</text>
        </view>
        <view class="relay-wrap wrap2 uni-column-center" @click="goodsForward">
          <image class="relay-img" src="/static/img/device/device_share2.png" mode=""></image>
          <text>转发</text>
        </view>
      </view>
    ...
    <view class="popup-layer" @touchmove.stop.prevent="discard" v-if="!hideMore">
      ...
      <view class="more-layer">
        <swiper class="swiper" :indicator-dots="indicatorBol">
          <swiper-item>
            <view class="list">
              <view class="box" v-for="(item, index) in serviceList.slice(0, 8)" :key="index" @click="serviceBtn(item)">
                <view class="icon">
          ...
          <swiper-item v-if="serviceList.length > 8">
            <view class="list">
              <view class="box" v-for="(item, index) in serviceList.slice(8, 16)" :key="index" @click="serviceBtn(item)">
                <view class="icon">
                  <image :src="item.image" mode=""></image>
                </view>
                <view class="text">{{ item.name }}</view>
          ...
          <swiper-item v-if="serviceList.length > 16">
            <view class="list">
              <view class="box" v-for="(item, index) in serviceList.slice(16, 24)" :key="index" @click="serviceBtn(item)">
                <view class="icon">
                  <image :src="item.image" mode=""></image>
                </view>
                <view class="text">{{ item.name }}</view>
...
export default {
  ...
  onLoad(e) {
    ...
    uni.$on('get_message', (obj) => {
      if (obj.roomid == that.roomid) {
        ...
        if (obj.content.type == '1' && obj.content.subtype == '9') {
          return
        } else {
          // 传消息给webview改变状态
  ...
  methods: {
    ...
    ttyToUrl(item) {
      ...
      } else if (item.content.subtype === '5') {
        setTimeout(() => {
    ...
    toVideo(item) {
      item.bol = !item.bol
      let content = `${item.bol ? `播放文件${item.content}` : '停止文件'}`
    ...
    // 文件翻页
    turnPage(action, item) {
      if (this.turnPageFileUrl === '' || this.turnPageFileUrl !== item.content) {
        this.turnPageFileUrl = item.content
        let content = `播放文件${item.content}`
        this.beforeSend(content)
      }
